...
# ==================================================================================================
        ...
        try:
            if self.var_roll.get()=="":
                ...
                if row!=None:
                    ...
                    cur.execute("insret into student(roll,name,email,gender,dob,contact,admission,course,state,city,pin,address) values(?,?,?,?,?,?,?,?,?,?,?,?)",(
                        self.var_roll.get(),
                        self.var_name.get(),
                        self.var_email.get(),
                        self.var_gender.get(),
    ...
    def search(self):
        ...
        try:
                ...
                cur.execute("select * from stdent where roll=?",(self.var_search.get(),))
                row=cur.fetchone()
                if row!=NONE:
                    self.CourseTable.delete(*self.CourseTable.get_children())
                    self.CourseTable.insert('',END,values=row)
